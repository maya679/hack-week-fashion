var bcr_mig = function (ins_id,ins_token) {
	...
	function bcr_mig_profile(user_id,ins_token) {
		$.ajax({
			...
			var html = '';
			html += '<div class="ins_pr_user"><img src="' + data.data.profile_picture.replace(/\\/, "")
								 + '" class="ins_pr_img" alt="' + data.data.username + '" title="' + data.data.username 
								 + ' on instagram"></div>';
								 // next>>>
			html += '<p class="ins_username"><a href="http://instagram.com/' + data.data.username
								+ '" target="_BLANK">' + data.data.username + '</a></p>';
								// next>>>
			html += '<p>Name: ' + data.data.full_name + '</p>';
								// next>>>
			html += '<p>Bio: ' + data.data.bio + '</p>';
								// next>>>
			html += '<p>Website: <a href="' + data.data.website + '" target="_BLANK">'
								+ data.data.website + '</a></p>';
								// next>>>
			html += '<p><strong>' + data.data.counts.media + '</strong> posts | <strong>' 
								+ data.data.counts.followed_by 
								+ '</strong> followers | <strong>'
								+ data.data.counts.follows + '</strong> following</p>';
								// next>>>
